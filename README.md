...
We use getAll() to get all the user's books. We store the state of all the books as a map of book id to book state, so that we can reuse this map to quickly look up book state. This is really useful for example for looking up the selection state
